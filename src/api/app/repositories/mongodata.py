...
client = MongoClient('mongodb://karyn:pass@mongo-project:27017/\
    ?authSource=
admin&authMechanism=SCRAM-SHA-1', connect=True)

- mongo local for tests connection
client = MongoClient('mongodb://localhost:27018/\
                      ?readPreference=primary&ssl=false', connect=True)
'''

client = MongoClient('mongodb://karyn:pass@mongo-project:27017/\
    ?authSource=admin&authMechanism=SCRAM-SHA-1', connect=True)
...
class FoodRepository():
    ...
    def list(self):
        return \
             operations.Cast_Many_to_Object(list(self.food_collection.find()))
