import datetime
from mocks.mock_operations import Initial_Check, Cast_Many_to_Object, Cast_One_to_Object

def test_Initial_Check():
    data_test = []
    assert (not data_test) == True

    data_test = Initial_Check(data_test)
    assert (not data_test) == False
...
def test_Cast_One_to_Object():
    ...
    assert (not data_test_obj) == False
...
def test_Cast_One_to_Object():
    ...
    data_test = None
    date_now = str(datetime.datetime.now())
    food_test = {"_id":'3',"name": "Gianduia", "origination":"Piemonte - Italy", "created":"1806 - 19th century","date": date_now}
